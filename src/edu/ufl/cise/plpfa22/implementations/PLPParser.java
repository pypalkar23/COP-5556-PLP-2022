...
import edu.ufl.cise.plpfa22.exceptions.SyntaxException;
...
public class PLPParser implements IParser {
    ...
    private List<ConstDec> parseConstantDec() throws PLPException {
        ...
        while (this.token.getKind() != Kind.EOF) {
            ...
            if (!isEqualToken()) {
                throw new SyntaxException
                        (String.format(CodeGenUtils.ParserUtils.WRONG_CHARACTER_DETECTED, CodeGenUtils.LexerUtils.EQ),
            ...
            constDecs.add(new ConstDec(firstToken, ident, CodeGenUtils.ParserUtils.getConstVal(this.token)));
            consume();
            if (isCommaToken())
                consume();
            if (isSemiColonToken()){
        ...
        if (!isSemiColonDetected)
            throw new SyntaxException(CodeGenUtils.LexerUtils.ERROR_REACHED_END_OF_FILE, token.getSourceLocation().line(), token.getSourceLocation().column());
    ...
    private List<VarDec> parseVariableDec() throws PLPException {
        ...
        while (this.token.getKind() != Kind.EOF) {
            if (!isIdentToken()) {
                throw new SyntaxException(CodeGenUtils.ParserUtils.SYNTAX_ERROR, token.getSourceLocation().line(), token.getSourceLocation().column());
            }
        ...
        switch (this.token.getKind()) {
            ...
            case LPAREN -> {
                ...
                if (this.token.getKind() != Kind.RPAREN) {
                    throw new SyntaxException(String.format(CodeGenUtils.ParserUtils.INVALID_CHARACTER_FOUND, CodeGenUtils.LexerUtils.RPAREN));
                }
            }
            default -> {
                throw new SyntaxException(String.format(CodeGenUtils.ParserUtils.INVALID_CHARACTER_FOUND, null));
            }
        }
        consume();
        return exp;
